...
date: "<div style= 'text-align:center ;font-size:24px;'>`r format(Sys.Date(), '%b-%y')` </span>"
format: 
  html:
...
source("helper.R")
 # import dataset
  ...
  assign("data_cpep", data_cpep, envir = .GlobalEnv)
  data_gluc <- read.csv("data/Glucose_Data_with_Missing_Values.csv")
  assign("data_gluc", data_gluc, envir = .GlobalEnv)
```
...
assign("data_cpep_imputed", data_cpep_imputed, envir = .GlobalEnv)
data_gluc_imputed <- read.csv("data/data_gluc_imputed.csv")
assign("data_gluc_imputed", data_gluc_imputed, envir = .GlobalEnv)
