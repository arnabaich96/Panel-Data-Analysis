...
server <- function(input, output, session) {
  ...
  output$Summary_imputed <- renderText({
    ...
    if (input$Treatment == "Pooled") {
      ...
      summary_table <- summary_table %>%
        kable("html", table.attr = "class='table table-striped'", caption = paste("Summary of Imputed Data:", input$SummaryType)) %>%
        kable_styling(full_width = F, bootstrap_options = c("striped", "hover", "condensed")) %>%
        as.character()
...
data$Time <- sapply(data$Time, convert_to_minutes)
  ...
  auc_ID <- auc_trapezoidal %>% filter(ID == input$selectedID) %>% ungroup()
        switch(input$Treatment,
               "Pooled" = {
                 ggplot(auc_trapezoidal, aes(x = Visit, y = auc)) +
                   ...
                   theme_classic()+
                   ggtitle(paste("Summary:",input$AUC_SummaryType,"of AUC"))
               },
               "Individual" = {
                 ggplot(auc_ID, aes(x = Visit, y = auc, group = ID)) +
                  ...
                  ggtitle(paste("AUC for subject ID:", input$selectedID))
               }
        )
      })
...
data$Time <- sapply(data$Time, convert_to_minutes)
  ...
  auc_ID <- auc_trapezoidal %>% filter(ID == input$selectedID)
      switch(input$Treatment,
             "Pooled" = {
               o=auc_trapezoidal %>% group_by(Visit) %>%  summarise(auc = match.fun(input$AUC_SummaryType)(auc, na.rm = TRUE))
               b=xtabs(auc ~ Visit, data = o)
               if(input$plotvariable=="Visit") b = b
               else b = t(b)
               knitr::kable(b, caption = paste("Summary:",input$AUC_SummaryType,"for AUC"))
             },
             "Individual" = {
               b =  xtabs(auc ~ Visit, 
               ...
               knitr::kable(b, caption = paste("AUC of subject ID:", input$selectedID))
...
data$Time <- sapply(data$Time, convert_to_minutes)
  ...
  auc_ID <- auc_trapezoidal %>% filter(ID == input$selectedID)
      switch(input$Treatment,
             "Pooled" = {
               o=auc_trapezoidal %>% group_by(Visit) %>% summarise_at(vars(auc), list(input$AUC_SummaryType))
               b=xtabs(auc ~ Visit, data = o)
               if(input$plotvariable=="Visit") b = b
               else b = t(b)
               knitr::kable(b, caption = paste("Summary:",input$AUC_SummaryType,"for AUC"))
