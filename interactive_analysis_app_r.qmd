      ...
      plot <- switch(input$plotvariable,
                     "Visit" = {
                       plot_ly(summary_data, x = ~Time, y = ~Measurement_mean, color = ~as.factor(Visit), type = 'scatter', mode = 'lines+markers') %>%
                         layout(
                           title = "Interaction Plot with Confidence Interval",
      ...
      if (input$CI) {
        plot <- plot %>%
          add_ribbons(
            ...
            line = list(color = 'transparent'),  # Hide the CI line
            fillcolor = ~paste0("rgba(", col2rgb("red")[1], ",", col2rgb("red")[2], ",", col2rgb("red")[3], ",0.2)"),  # Adjust with a fixed color and transparency
            hoverinfo = "none",  # Remove CI hover info
      ...
      plot <- switch(input$plotvariable,
                     "Visit" = {
                       plot_ly(plot_data, x = ~Time, y = ~Measurement, color = ~as.factor(Visit), type = 'scatter', mode = 'lines+markers') %>%
                         layout(
                           title = paste("Interaction Plot for ID:", input$selectedID),
                           xaxis = list(title = "Time"),
                           yaxis = list(title = "Measurement"),
                           legend = list(orientation = 'h', x = 0.5, xanchor = 'center', y = -0.2)
                         )
    ...
    if (input$Treatment == "Pooled") {
      ...
      plot <- switch(input$plotvariable,
                     "Visit" = {
                       plot_ly(summary_data, x = ~Time, y = ~Measurement_mean, color = ~as.factor(Visit), type = 'scatter', mode = 'lines+markers') %>%
                         layout(
                           title = "Imputed Data Interaction Plot with Confidence Interval",
                           xaxis = list(title = "Time"),
                           yaxis = list(title = "Measurement"),
                           legend = list(orientation = 'h', x = 0.5, xanchor = 'center', y = -0.2)
                         )
