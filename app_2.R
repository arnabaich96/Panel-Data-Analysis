  ...
  output$InteractionPlot_raw <- renderPlotly({
    ...
    if (input$Treatment == "Pooled") {
      summary_data <- plot_data %>%
        ...
        arrange(as.numeric(time)) %>%  # Ensure time is in increasing order
        ungroup()

      plot <- switch(input$plotvariable,
                     "Visit" = {
                       plot_ly(summary_data, x = ~as.numeric(time), y = ~Measurement_mean, color = ~as.factor(visit), type = 'scatter', mode = 'lines+markers') %>%
                         layout(
                           xaxis = list(title = "Hour", tickvals = sort(unique(as.numeric(time))), ticktext = paste0(sort(unique(as.numeric(time))), "hr")),
                     ...
                     "Time" = {
                       plot_ly(summary_data, x = ~as.numeric(visit), y = ~Measurement_mean, color = ~as.factor(time), type = 'scatter', mode = 'lines+markers') %>%
                         layout(
      ...
      plot_data <- plot_data %>% filter(subject == input$selectedID) %>% arrange(as.numeric(time))
      plot <- switch(input$plotvariable,
                     "Visit" = {
                       plot_ly(plot_data, x = ~as.numeric(time), y = ~result, color = ~as.factor(visit), type = 'scatter', mode = 'lines+markers') %>%
                         layout(
                           xaxis = list(title = "Hour", tickvals = sort(unique(as.numeric(time))), ticktext = paste0(sort(unique(as.numeric(time))), "hr")),
                           yaxis = list(title = paste(input$Measurement)),
                           legend = list(orientation = 'h', x = 0.5, xanchor = 'center', y = -0.2),
                     ...
                     "Time" = {
                       plot_ly(plot_data, x = ~as.numeric(visit), y = ~result, color = ~sort(as.factor(time)), type = 'scatter', mode = 'lines+markers') %>%
                         layout(
                           xaxis = list(title = "Visit"),
                           yaxis = list(title = paste(input$Measurement)),
                           legend = list(orientation = 'h', x = 0.5, xanchor = 'center', y = -0.2),
  ...
  output$InteractionPlot_imputed <- renderPlotly({
    ...
    if (input$Treatment == "Pooled") {
      ...
      plot <- switch(input$plotvariable,
                     ...
                     "Time" = {
                       plot_ly(plot_data, x = ~as.numeric(visit), y = ~result, color = ~as.factor(time), type = 'scatter', mode = 'lines+markers') %>%
                         layout(
                           xaxis = list(title = "Visit"),
                           yaxis = list(title = paste(input$Measurement)),
                           legend = list(orientation = 'h', x = 0.5, xanchor = 'center', y = -0.2),
